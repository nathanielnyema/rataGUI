...
class CameraWidget(QtWidgets.QWidget, Ui_CameraWidget):
    ...
    def __init__(self, camera=None, cam_config=None, plugins=[], triggers=[], sessionID=""):
        super().__init__()
        self.setupUi(self)
        ...
        self.save_dir = os.path.join(launch_config["Save Directory"], sessionID, slugify(camera.getDisplayName()))
        ...
        for Plugin, config in plugins:
            try:
                ...
                self.failed_plugins[plugin_name] = config_dict
                logger.exception(err)
                logger.error(f"Plugin: {plugin_name} for camera: {self.camera.getDisplayName()} failed to initialize")
